...
class Web:
    ...
    def init_web(self):
        self.app.mount("/assets", StaticFiles(directory="pagermaid/assets"), name="assets")
        ...
        async def login():
            return login_page.render(
                site_title="登录 | Soluna-Gram 后台管理",
                site_icon=icon_path,
            )
    ...
    async def start(self):
        ...
        if not Config.WEB_SECRET_KEY:
            logs.warning("未设置 WEB_SECRET_KEY ，请勿将 Soluna-Gram 暴露在公网")
        import uvicorn
