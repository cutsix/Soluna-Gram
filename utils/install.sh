#!/bin/bash
SOLUNA_REPO_URL="${SOLUNA_REPO_URL:-}"
SOLUNA_REPO_BRANCH="${SOLUNA_REPO_BRANCH:-release}"
...
welcome() {
    echo "一键脚本如有异常，请优先回到仓库 README 手动部署。"
    echo "一键脚本如有异常，请优先回到仓库 README 手动部署。"
    echo "一键脚本如有异常，请优先回到仓库 README 手动部署。"
    echo ""
    echo ""
    echo "欢迎使用 Soluna-Gram 一键安装程序。"
    echo "安装即将开始"
    echo "如果您想取消安装，"
...
configure() {
    ...
    [yY][eE][sS] | [yY])
        printf "请输入您的日志记录群组/频道的 ChatID（不填则发送到 Saved Messages）："
        read -r log_chatid <&1
        if [ -z "$log_chatid" ]; then
            echo "LOG 将发送到 Saved Messages."
        else
...
login_screen() {
    ...
    while :; do
        echo "一键脚本出现任何问题请转手动搭建！ 仓库 README"
        echo "一键脚本出现任何问题请转手动搭建！ 仓库 README"
        echo "一键脚本出现任何问题请转手动搭建！ 仓库 README"
        echo ""
...
systemctl_reload() {
    echo "正在写入系统进程守护 . . ."
    echo "[Unit]
    Description=Soluna-Gram telegram utility daemon
...
start_installation() {
    if [ "$release" = "centos" ]; then
        ...
        login_screen
        systemctl_reload
        echo "Soluna-Gram 已经安装完毕 在telegram对话框中输入 ,help 并发送查看帮助列表"
...
shon_online() {
    ...
    echo ""
    echo "请选择您需要进行的操作:"
    echo "  1) 安装 Soluna-Gram"
    echo "  2) 卸载 Soluna-Gram"
    echo "  3) 重新安装 Soluna-Gram"
    echo "  4) 重新登陆 Soluna-Gram"
    echo "  5) 关闭 Soluna-Gram"
    echo "  6) 启动 Soluna-Gram"
    echo "  7) 重新启动 Soluna-Gram"
