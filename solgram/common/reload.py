...
from solgram.common.plugin import plugin_manager
from solgram.hook import Hook
...
async def reload_all():
    ...
    loaded_plugins = list(solgram.modules.plugin_list)
    loaded_plugins.extend(iter(solgram.modules.module_list))
    # init
    importlib.reload(solgram.config)
    importlib.reload(solgram.modules)
    help_messages.clear()
    all_permissions.clear()
    for functions in hook_functions.values():
        functions.clear()  # noqa: clear all hooks

    for module_name in solgram.modules.module_list:
        try:
            module = importlib.import_module(f"solgram.modules.{module_name}")
    ...
    for plugin_name in solgram.modules.plugin_list.copy():
        try:
            ...
            solgram.modules.plugin_list.remove(plugin_name)
...
async def load_all():
    for module_name in solgram.modules.module_list.copy():
        try:
            importlib.import_module(f"solgram.modules.{module_name}")
        except BaseException as exception:
            logs.info(
                f"{lang('module')} {module_name} {lang('error')}: {type(exception)}: {exception}"
            )
